...
class CustomMetrics:
    """Classe per il calcolo di metriche personalizzate"""
    ...
    def calculate_custom_metrics(query, answer, contexts):
        ...
        query_words = set([w.strip() for w in query_lower.split() if w.strip()])
        answer_words = set([w.strip() for w in answer_lower.split() if w.strip()])
        ...
        context_words = set([w.strip() for w in combined_context.lower().split() if w.strip()])
        ...
        try:
            ...
            if answer_words and context_words:
                ...
                context_coverage = answer_in_context / len(answer_words) if answer_words else 0
            ...
            if query_words and answer_words:
                ...
                query_coverage = query_covered / len(query_words) if query_words else 0
            ...
            if answer and answer.strip():
                ...
                length_score = min(answer_length / optimal_length, 1.0) if optimal_length > 0 else 0
                custom_results['answer_length_score'] = round(length_score, 4)

            # 6. Technical Terms Density
            if answer and answer.strip():
                words = answer.split()
                long_words = [w for w in words if len(w) > CUSTOM_METRICS_CONFIG['technical_word_min_length']]
            ...
            if query_words and context_words:
                ...
                context_relevance = query_in_context / len(query_words) if query_words else 0
                custom_results['context_relevance'] = round(context_relevance, 4)

            # 8. Answer Completeness
            if answer and query:
                answer_completeness = min(len(answer) / CUSTOM_METRICS_CONFIG['answer_completeness_cap'], 1.0)
                custom_results['answer_completeness'] = round(answer_completeness, 4)
